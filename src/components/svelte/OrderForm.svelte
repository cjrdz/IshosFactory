<script lang="ts">
    ...
    $effect(() => {
        ...
        if (initialData.orderType !== undefined)
            formData.orderType = initialData.orderType || "pickup";
        if (initialData.deliveryNotes !== undefined)
            formData.deliveryNotes = initialData.deliveryNotes || "";
    ...
    function saveCustomerInfo() {
        if (typeof window !== "undefined") {
            if (formData.name)
                localStorage.setItem("ishos_customer_name", formData.name);
            if (formData.phone)
                localStorage.setItem("ishos_customer_phone", formData.phone);
            if (formData.email)
                localStorage.setItem("ishos_customer_email", formData.email);
            if (formData.address)
                localStorage.setItem(
                    "ishos_customer_address",
                    formData.address,
                );
        }
    }
    ...
    function handleSubmit(e: Event) {
        ...
        const allNotes =
            [formData.deliveryNotes, formData.notes]
                .filter(Boolean)
                .join(" | ") || undefined;
...
<form onsubmit={handleSubmit} class="space-y-4">
    <div class="form-control">
        <label class="label" for="order-name">
    ...
    <div class="form-control">
        <label class="label" for="order-phone">
    ...
    <div class="form-control">
        <label class="label" for="order-email">
    ...
    <div class="form-control">
        <fieldset class="border-0 p-0 m-0">
            <legend id="order-type-legend" class="label">
                <span class="label-text">Tipo de pedido *</span>
            </legend>
            <div
                class="flex gap-4"
                role="radiogroup"
                aria-labelledby="order-type-legend"
            >
    ...
    {#if formData.orderType === "delivery"}
        <div class="form-control">
            <label class="label" for="order-address">
        ...
        <div class="form-control">
            <label class="label" for="order-delivery-notes">
                <span class="label-text"
                    >Instrucciones de entrega (opcional)</span
                >
            </label>
            <textarea
    ...
    <div class="form-control">
        <label class="label" for="order-notes">
