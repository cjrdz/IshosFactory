<script lang="ts">
    ...
    onMount(() => {
        const unsubscribeCart = cart.subscribe((value: any) => {
            cartStore = value;
        });
        const unsubscribeTotal = cartTotal.subscribe((value: any) => {
            total = value;
        });
        return () => {
            unsubscribeCart();
        ...
        <div class="space-y-2">
            <div class="flex justify-between">
                <span
                    ...
                    {summary.itemCount === 1 ? "artículo" : "artículos"})</span
                >
            ...
            {#if showTax && taxRate > 0 && summary.tax > 0}
                <div class="flex justify-between">
                    ...
                    <span class="font-semibold"
                        >{formatPrice(summary.tax, "USD", currencySymbol)}</span
                    >
                </div>
            {/if}
            <div class="divider"></div>
            <div class="flex justify-between text-lg font-bold">
                ...
                <span class="text-primary"
                    >{formatPrice(summary.total, "USD", currencySymbol)}</span
                >
            </div>
        </div>
    </div>
